...
/**
 * Check if the report is valid with tolerance.
 *
 * @param report The report to check.
 *
 * @return `true` if the report is valid with tolerance, `false` otherwise.
 */
private fun isReportValidWithTolerance(report: List<Int>): Boolean =
    (0..report.lastIndex)
        .map { indexToRemove -> report.filterIndexed { index, _ -> index != indexToRemove } }
        .any { dampenedReport -> isReportValid(dampenedReport) }
